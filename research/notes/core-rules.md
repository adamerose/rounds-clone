...
Worst-case propagation of ±0.5 px per core and stable-reference coordinate, ±1 px player diameter, and exact endpoint timing yields the table's two intervals. The arithmetic mean of the two point estimates is `0.38017235`; rounded to the binding's two-decimal precision, the principled common candidate is `0.38`. Before ticket 030's binding phase, the clone's `2.4` value was outside both intervals while `0.38` was inside both. The first evidence commit preserved `2.4` to keep measurement and source changes separate; the later binding commit applied `0.38` to the embedded combat fact and runtime and regenerated the deterministic replay consequences.
...
The present movement, jump, damage, fire, reload, recoil, block, projectile, and arena values remain development scaffolding until direct installed-build comparisons under tickets 016–018 replace or validate them. Ticket 030 corrected the retained projectile measurement and then changed the shared `spec/combat.json`/runtime binding from `2.4` to `0.38`; its regenerated golden replay now ends at `033f39aa612bf67b`. Tickets 016 and 031 still own confirmation against installed public build `21020021`, so this result remains a retained-footage correction rather than a complete current-build calibration.
