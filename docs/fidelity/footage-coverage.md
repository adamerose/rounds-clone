...
| `S6-match` | reproduce round, half, score, color handoff, match completion and rematch cadence | blue 4–5 victory/rematch/0–0 reset continues through the first full-round award at recording `453954a7…a18c` PTS 2506156642, but that terminal 4–5 is constructed prior state rather than reproduced behavior: `FlowAuthority::new` hard-codes `scores: [4, 5]` with `winner: Some(1)` and `eliminated: Some(0)`, and no score-driven match award exists anywhere in the flow — `record_elimination` awards halves and completed rounds only, and no phase or rule ends a match on a completed-round count; either color may sweep the opening fights or win the deciding ice duel; a same-tick double elimination provisionally repeats the fight without an award because neither recording shows one; match completion, next draft and remaining lifecycle unresolved |
| `S7-presentation` | reproduce characters, lighting, camera, shake, trails, hit-stop, chromatic/radial effects, particles, text and audio | first static-duel, explosive-collapse and connected ice/first-round visual sub-slices implemented in the shared scene; the radial-saw scene from ticket 042 (rotated platform and saw poses read from the snapshot, tick-derived paper-brush motion, long shadows, result dimming, half-score circles, `HALF BLUE`) and the yellow-crate scene from ticket 043, whose one private final-composite fullscreen pass carries the source-proved discrete radial RGB echoes, are implemented as well; audio, hit-stop, and remaining presentation unresolved |
...
| 03:50–04:00 | elimination and HALF ORANGE through PTS 2351823926, then incoming ice and first traversal | connected S2/S5/S6/S7 route implemented, with the ice traversal a named gap: at the bound 4786 anchor the clone's blue stands 95.3 native pixels right and 35.8 pixels lower than the source's blue, per ticket 049's Decisions against the re-tracked source body, and the residual is the airborne approach rate rather than a top-speed shortfall, because ticket 052 fits the source's sustained airborne top speed at 218.2–219.4 px/s against the clone's 192.0 px/s fixed point while the source converges at 0.605–0.815 per tick against the clone's 0.9094; both tickets are `blocked`, so both source-side figures are provisional and re-checked when either lands; unobserved ice reactions and production S8 unresolved |
...
| 07:00–07:10 | yellow-crate duel, terminal blast/radial screen echo through final combat PTS 4238316380, adjacent result onset at PTS 4238483046, then `ROUND ORANGE` | S2/S4/S6/S7 implemented sub-slice; the profile carries no card state at all, reporting `flow: null` and `flowDigest: null`, so S5 card behavior is unresolved, and its two localhost development-transport UDP clients leave production S8 unresolved; other yellow-arena combat remains incomplete |
...
| 03:20–03:30 | match end: `WAITING` over the still-live arena, both fighters visible, three filled orange pips against five filled blue; PTS 2000158666, RGBA `c4c9547151263157cf54afe9495c7f1b1103cc3c2da8d3b29314bb0c57a09a6d`. New-match reset: both five-pip rows empty, no loadout badges at the upper right, fresh orange five-card draft fan mid-reveal; PTS 2039991840, RGBA `7f8703810079f5beef741953d896175d70ee5602fe997b37be3349308c218da0`. The only match end on a score and following new match in either recording, and so the anchor for S6's missing match award and S1's new-match draft | S1, S6, S7 |
