...
| `S1-flow-draft` | reproduce card offers, readable card faces, player reveal, pick input, inter-round handoff, waiting and rematch screens | opening rematch/two-draft and first loser-only post-round draft implemented through native PTS 2577323024; remaining flows unresolved |
...
| `S6-match` | reproduce round, half, score, color handoff, match completion and rematch cadence | blue 4–5 victory/rematch continues through first-round award, loser-only draft and half reset at PTS 2510823290; both winner colors are authority-tested; later rounds and match completion remain unresolved |
...
`S6-match` begins from a hard-coded prior match state: the flow authority is constructed with `scores: [4, 5]`, `winner: Some(1)` and `eliminated: Some(0)` in `crates/rounds-sim/src/flow.rs`, so the terminal 4–5 the rematch route displays is where the authority starts rather than a score any round drove it to. No score-driven match award exists anywhere in the flow: `record_elimination` awards halves and completed rounds only, and no phase or rule ends a match on a completed-round count.
