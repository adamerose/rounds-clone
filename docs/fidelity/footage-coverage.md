...
| `S1-flow-draft` | reproduce card offers, readable card faces, player reveal, pick input, inter-round handoff, waiting and rematch screens | opening rematch/two-draft, first loser-only post-round draft and score-driven `WAITING` implemented; the following new-match reset remains unresolved |
...
| `S5-card-combat` | reproduce every visible named card and its stacked combat interaction across rounds | Dazzle, Explosive Bullet and Quick Shot's typed launch-speed seam implemented; Cold Bullets, Careful Planning, Overpower and Big Bullet remain catalog-only, and Quick Shot magnitude/reload remain unresolved |
| `S6-match` | reproduce round, half, score, color handoff, match completion and rematch cadence | first-to-five match completion is authority-driven and source-bound at the 3–5 `WAITING` frame; later new-match reset and remaining round cadence are unresolved |
| `S7-presentation` | reproduce characters, lighting, camera, shake, trails, hit-stop, chromatic/radial effects, particles, text and audio | first static-duel, explosive-collapse and connected ice/first-round visual sub-slices implemented in the shared scene, along with ticket 042's radial-saw scene and ticket 043's yellow-crate scene; audio, hit-stop, and remaining presentation unresolved |
...
The connected rematch route joins the existing draft, timber and ice slices from native PTS 1595160286 through the background-only post-round bridge at PTS 2577323024. The blue/orange/blue sequence enters ice with halves 1–1 and rounds 0–0, reaches halves 1–2 and rounds 0–1, then resets halves to 0–0 while retaining the completed round and `Da`/`Ex`. Orange, the completed-round loser, alone drafts Quick Shot and its stack becomes `Da Qu`. The footage cannot distinguish “round loser” from “trailing player” or “last-fight loser”; that ambiguity is not claimed resolved.
The ice extension starts at PTS 2351990592. Its seventeen static polygon contours supply both collision and rendering, while animated cyan/pale paint, shadows and incoming/outgoing motion stay in the shared presentation scene. The interval provides no evidence for ice friction, fracture or melting. The connected route then leaves the tick-5893 bridge for a held hanging-arena entry through tick 5941. Its typed 21-body layout and 42 visible links are presentation geometry only; separate left, right, and central curves settle by tick 5918 while both fighters remain at measured zero-velocity poses. It adds no suspension topology, collision behavior, control release, reload, ammunition, projectile, or damage claim. See `connected-match-observations.md`, `ice-round-observations.md`, `first-loser-draft-observations.md`, and `held-hanging-entry-observations.md` for the tracked measurements. The existing `rematch-draft-replay` profile accepts the complete 5,941-tick route for local controls, bounded playback, replay capture and the two-client smoke path; the earlier 5,893-tick run remains the pre-arena endpoint. This adds no separate profile or production-network claim.

```text
out/cargo-target/debug/rounds-automation inspect --profile rematch-draft-replay --seed 41 --ticks 5941
out/cargo-target/debug/rounds-client capture-replay --profile rematch-draft-replay --seed 41 --ticks 5941 --output-dir out/ticket-055/anchors --metadata out/ticket-055/anchors.json
out/cargo-target/debug/rounds-automation smoke --profile rematch-draft-replay --seed 41 --ticks 5941 --output-dir out/ticket-055/smoke
out/cargo-target/debug/rounds-client visible-flow --profile rematch-draft-replay --seed 41 --ticks 5941 --automated
```

The connected rematch route explicitly constructs its historical 4–5 result, while fresh flow construction starts live at 0–0. The separate `match-end-waiting-replay` labels its 3–4, one-half-each prehistory as constructed, then reaches 3–5 through an ordinary projectile elimination and the normal result/round envelope before entering stable `WAITING`. It binds score, phase, badges and visible fighters to PTS 2000158666 without claiming the replay arena or route matches the pink-fortress source. PTS 2039991840 proves a cleared 0–0 draft follows, but its unseen trigger remains unresolved.
The two presentation scenes named in `S7-presentation` are ticket 042's radial-saw scene — rotated platform and saw poses read from the snapshot, tick-derived paper-brush motion, long shadows, result dimming, half-score circles and `HALF BLUE` — and ticket 043's yellow-crate scene, whose one private final-composite fullscreen pass carries the source-proved discrete green, yellow and red echoes.
...
| 07:00–07:10 | yellow-crate duel, terminal blast/radial screen echo through final combat PTS 4238316380, adjacent result onset at PTS 4238483046, then `ROUND ORANGE` | S2/S4/S6/S7 implemented sub-slice; S5 card behavior, production S8 and other yellow-arena combat remain incomplete |
...
The 03:50–04:00 ice traversal gap carries current figures on both sides. At the bound 4786 anchor the clone's blue stands 95.3 native pixels right and 35.8 pixels lower than the source's blue, per ticket 049, which measured against the re-tracked source body at (640.0, 368.5); the older pair in `ice-round-observations.md` was measured against that record's earlier estimate and is superseded.
The cause is the airborne approach rate rather than a top-speed shortfall: ticket 052 fits the source's sustained airborne top speed at 218.2–219.4 px/s against the clone's 192.0 px/s fixed point, while the source converges at 0.605–0.815 per tick against the clone's 0.9094. Both tickets are blocked, so both source-side figures stay provisional and are re-checked when either lands.
The 07:00–07:10 yellow-crate sub-slice comes from ticket 043, which delivered two development-transport UDP client processes on the localhost adapter and no card formula at all: `rounds-automation inspect` reports the `yellow-crate-terminal-blast-replay` profile with `flow: null` and `flowDigest: null`, so it carries no item, loadout or capability state whatever. The row keeps S6 because that same sub-slice does own an authority-driven blue elimination, orange score and `ROUND ORANGE` result.
...
| 03:20–03:30 | the observed match end on a completed-round score, `WAITING` over the live arena at PTS 2000158666, then the new-match draft that follows at PTS 2039991840; anchors S6's missing match award and S1's new-match draft; frame identities below | S1, S6, S7 |
...
The two 03:20–03:30 frames were decoded at native 1280×720. PTS 2000158666, RGBA `c4c9547151263157cf54afe9495c7f1b1103cc3c2da8d3b29314bb0c57a09a6d`, shows `WAITING` over the still-live arena with both fighters visible and the HUD at three filled orange pips against five filled blue pips. PTS 2039991840, RGBA `7f8703810079f5beef741953d896175d70ee5602fe997b37be3349308c218da0`, shows both five-pip rows empty, no loadout badges at the upper right and a fresh orange five-card draft fan mid-reveal.
