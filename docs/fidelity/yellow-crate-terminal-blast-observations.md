...
The replay capture emits the eleven table anchors above at 1280x720. Its metadata binds PTS 4238316380 to tick 109 as `last-combat`, adjacent PTS 4238483046 to tick 110 as `result-onset`, and PTS 4238649712 to tick 111 as `following-result-transition`, along with every anchor's exact source PTS/native RGBA hash, authoritative digests, executable and clone-frame hashes, renderer identity `bevy-0.19.1-2d-hdr-shared-scene-single-final-composite-radial-echo`, single-pass identity, tick-81 audio hook, and actual composited-GPU measurements. Those measurements protect the restrained onset, in-frame local core, hard arena and HUD edges through the echo, trail envelope, small half-filled orange onset, larger following transition, and late winner-circle contraction from regressing to the rejected soft zoom smear. Original-resolution source-left/clone-right inspection found the same 4/5/4 arena silhouette, progressive top-right approach, readable upper-right blast, whole-frame outward echo direction, several hard green/yellow/red copies, narrow multicolor trail direction, detached crate, abrupt result cut, score circles, and result cadence. The clone's paper facets and particle lobes remain source-shaped approximations rather than proprietary texture or shader bytes.
...
## Fighter-contact correction during the connected ice delivery

Ticket 046 corrected the shared fighter-contact boundary: hit and explosion positions now use the struck surface instead of the projectile's later endpoint. In this replay, the tick-81 center moves from (453.645, 292) to (460.446, 292) in simulation coordinates. The ordinary shot, elimination, published fighter impulse and result boundaries remain intact. The changed radial origin changes crate impulses and subsequent collisions; maximum crate translation differences against the previous implementation are 6.90 world units at tick 85, 30.10 at 100 and 69.31 by the result. These are simulation-coordinate differences, not measured image pixels.

All eleven native source hashes were reproduced and all eleven before/after GPU frames inspected at 1280×720. The burst moves about seven image pixels right. The new path retains one upper-right resting piece with two detached pieces near the last combat frame, where the previous path had three clearly detached pieces; the source shows one detached piece and two remaining. Both paths reproduce a dynamic stack response, with approximate trajectories. The compact onset, white-hot burst, discrete radial copies, directional trails and adjacent result transition remain visible. Existing differences persist in echo displacement, the dense circular spark cloud, fighter spacing and exact geometry. Detached pieces stop with the result in this bounded replay while the source piece continues moving; the correction does not claim that later motion complete.

Retained artifacts are `out/ticket-046/yellow-source-verification.json`, `yellow-before-contact/`, `yellow-after-contact/` and `yellow-contact-state-comparison.json`. The original ice reproduction and recovery are `fighter-contact-before.txt` and `fighter-contact-after.txt`, using `probe.rs` and the legal actions in `late-terminal-close.txt`. The compact public regression is the Yellow replay's surface-distance and shared explosion-center assertion.
