...
- `Waiting` retains the final completed-round score, winner, badge stacks, loadouts, capabilities, and the live arena. Both fighters are visible and alive in the flow and ECS snapshots. Combat, draft, and rematch commands are rejected, further eliminations cannot score, and ordinary advancement leaves the state stable apart from its saturating phase age.
...
- Reviving both ECS fighters on entry makes the flow's `fighter_alive` claim and the source-visible pair true without respawning or changing the arena. Clear projectiles and transient draft focus/selection so no stale combat or card interaction remains visible; retain score and builds exactly.
...
- A public `AuthoritativeMatch::step` regression for `match-end-waiting-replay` proves a legal input projectile causes the deciding elimination, the score becomes 3–5 exactly once, both ECS fighters are alive and visible on entry, projectiles are gone, later movement/fire/flow inputs cannot change gameplay state, and no private winner or score mutation occurs after construction.
