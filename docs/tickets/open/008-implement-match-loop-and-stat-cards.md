...
- Give a player at most five acquired cards.
  If a full-round loser already has five, skip their draft but still change arena and start the next duel.
...
- After a loser confirms, choose the next arena from the ordinal eligible list with the current arena removed, apply all persistent player stats, and reset the already-incremented fresh duel without incrementing its number again.
  A skipped max-card draft performs those same arena and reconfiguration steps immediately.
...
- Draft tests prove deterministic seed-to-offer behavior, five distinct choices, owned-card recurrence, neutral arming, rising-edge wrap and confirmation, inactive-player lockout, sequential opening pickers, maximum-card skip, explicit PCG consumption, and changed offers for a changed seed.
