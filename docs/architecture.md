...
`PlayerInput::with_progressive_observation` resolves explicitly requested opponent-relative aim using the latest snapshot before input submission. The public `AuthoritativeMatch::snapshot` and `step` methods let a program choose and submit later actions without reconstructing state. The local trace, visible client-host and UDP clients use that same resolver and revisioned `FlowCommand` boundary. Snapshot protocol 8 carries the post-round phases, five added catalog entries (14 total), typed per-fighter projectile-speed factor, polygon contours, separate half/round state, and the presentation-only hanging-entry layout; network protocol 9 carries those snapshots and permits at most 6,000 exchanged ticks per smoke session. The bounded source route uses 5,941 ticks and ends on the last retained held hanging-arena observation.
