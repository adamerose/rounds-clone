...
The same rematch authority now continues after the original 2,400-tick draft slice. Ordinary player hits record the first winner and half award, an authority-owned result interval loads the timber arena, and the second combat ends in another hit-driven half award. One `AuthoritativeMatch`, its ECS world and its Rapier boundary survive the handoff. Existing fighters retain identity and selected capabilities; loading the next arena replaces its collision surfaces, revives and repositions both fighters beneath the outgoing result overlay, clears old projectiles, and creates the existing timber bodies and constraints in that world. The prior winner and half award remain visible until combat resumes.
The source trace produces blue then orange and a 1–1 half state. Live actions may produce either winner. Neither the authority nor presentation injects those two wins at a named replay tick.
