...
## 2026-09-05 — An output duration bound discarded timestamp-preserved source frames

Ticket 046's first headless source probe combined `-copyts` near source second 235 with an output `-t 21`. FFmpeg reported a selected source frame but emitted no raw frame bytes, so the audit's byte-count assertion rejected the result. Moving the short duration bound before `-i` made it an input bound; retaining `-frames:v` bounded output as well. The corrected overview and short transition scans decoded native frames successfully without scanning the remaining recording. Timestamp-preserved source audits must place duration bounds on the input, not compare source timestamps against a short output duration.

## 2026-09-06 — Arena cleanup changed an earlier contact trajectory

The first ice implementation removed the first arena's static Rapier bodies while loading timber. The existing connected regression then missed its ordinary second elimination. Those old colliders already had collision membership and filters set to `Group::NONE`; they were not providing invisible support. Removing their bodies before inserting the timber floor changed Rapier handle allocation and the resulting contact trajectory. Restoring the established timber insertion sequence restored the exact earlier impact and result ticks. This preserves disabled bodies only until the next arena handoff, where the new regression checks that every retired collider identity is gone.

The deeper ice cleanup check also found one fixed body without a collider: the timber joints' shared anchor. Its handle had never been retained. Tracking that anchor and removing it with the outgoing assembly leaves exactly seventeen ice bodies and two fighter bodies, nineteen colliders, no joints and no retired projectiles. The full source trace and all four ordinary-damage sweep/split outcomes still pass after that removal.

The first complete GPU pass exposed separate presentation failures: arena translation moved both revived fighters offscreen at entry, fixed-length shadow rays outlasted the departing arena, and the yellow-crate flash envelope delayed the ice terminal flash. The correction carries the actual outgoing fighter positions through the entry snapshot, ends shadow rays below the arena, and starts the reused flash envelope at its bright phase while letting elimination fragments descend. Source comparison also exposed substantial intermediate action-path differences despite correct terminal timing; those required further ordinary-input tuning before final evidence.

Two attempts by the implementation child to resume its existing trace child returned `agent thread limit reached`, including after the parent reported a free slot. The parent resumed that same canonical child successfully from the owning context. Work continued in the existing worktree and public-input harness; no agent deadline, forced cancellation or alternate-provider workaround was used.

The parent later reported the same refusal when resuming a separate source-only research child, despite an inventory showing three running contexts. That research continuation did not start; the active ice trace work continued. The visible agent inventory therefore did not reliably predict whether a continuation could be admitted.


## 2026-09-06 — Fighter contact effects used the projectile's later position

The ice traversal correction exposed a close vertical shot whose ordinary hit eliminated orange at tick 5312, but the effect appeared about 69 world units above the fighter. The hit predicate already detected Rapier or swept contact; the caller discarded that contact and used the projectile position at the end of the physics step. An 81-tick public Yellow replay reproduced the same coordinate error without the long ice setup. Its added surface-distance assertion failed before the correction.

The private contact boundary now returns the fighter's Rapier solver point, or the first swept-circle contact under the existing hit tolerance. The impact and its explosion use that point. Replaying the original vertical ice inputs now places the effect on orange's radius-12 surface with unchanged fighter positions, damage and result ticks. All 26 simulation tests pass, including either-color sweeps and splits. The Yellow explosion moves 6.801 world units horizontally, which changes crate impulses and later contacts; by the result, one crate differs by 69.31 world units. This is a physical correction as well as an event-coordinate correction. Eleven native source pairs confirm the terminal burst, discrete echo, directional trails and adjacent result sequence remain supported, while crate trajectories remain approximate.


## 2026-09-06 — Windows text defaults damaged a research log

The parent research task read UTF-8 text through Python's default Windows cp1252 encoding, which mangled punctuation in the next draft's research commit. A subsequent read failed with `UnicodeDecodeError`. The parent restored the original UTF-8 text from its prior commit, retained the new research entries, wrote explicit UTF-8 with LF and verified both the absence of damaged punctuation and the ticket/whitespace checks. The ice child also encountered a decode refusal before writing its postmortem; explicit UTF-8 restored that operation without changing the existing text. All Windows Python text reads and writes in this work should specify `encoding="utf-8"` rather than relying on the process default.

## 2026-09-06 — Native ice comparison exposed shared effect assumptions

The complete native comparison found broad cyan coverage, false navy ledges beneath the central cap, an upward terminal framing error, overly fast falling fragments and a fresh oversized platform burst. The shadow code used bounding width instead of the polygon's actual top vertices; the shared lens zoom and periodic camera motion also displaced ice geometry away from the observed contact. One correction pass narrowed the paint, used the true top silhouette, removed ice lens zoom, applied a brief event-relative downward pulse and slowed pale/orange fragments. An earlier ordinary shot aged the intermediate platform effect before its anchor.

The final twelve native pairs confirm that the false ledges are gone, the terminal core is much closer to the source and late fragments remain near the encounter. They also expose the tradeoff: the older intermediate burst is now about 33 screen pixels below its source halo, and central cyan bands remain too broad. The full residual inventory stays in the fidelity record for independent review; passing transport and scoring checks did not establish visual fidelity. All 37 GPU anchors, full UDP smoke and hidden-first visible playback were regenerated on the corrected executable.
